...
func DeleteExpiredLBs(sessions *ScalewaySessions, options *ScalewayOption) {
	...
	count, start := utils.ElemToDeleteFormattedInfos("expired load balancer", len(expiredLBs), region)
...
func listLBs(lbAPI *lb.API, tagName string) ([]ScalewayLB, string) {
	...
	for _, lb := range result.LBs {
		creationDate, ttl, isProtected, _, _ := utils.GetEssentialTags(lb.Tags, tagName)
...
func getExpiredLBs(lbAPI *lb.API, tagName string) ([]ScalewayLB, string) {
	...
	for _, lb := range lbs {
		if utils.CheckIfExpired(lb.CreationDate, lb.TTL, "load balancer"+lb.Name) && !lb.IsProtected {
